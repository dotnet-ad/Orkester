namespace Orkester
{
	...
	public interface IOperation<T>
	{
		...
		Func<dynamic, CancellationToken, Task<T>> ExecuteAsync { get; }
		...
		IOperation<IEnumerable<T>> WithRepeat(int times);
		...
		/// <param name="span">Span.</param>
		IOperation<T> WithAggregation(TimeSpan span);
		...
		/// <param name="span">Span.</param>
		IOperation<T> WithExpiration(TimeSpan span);

		/// <summary>
		/// Registers the task for later executions.
		/// </summary>
		/// <param name="name">Name.</param>
		IOperation<T> Save(string name);
	}
}